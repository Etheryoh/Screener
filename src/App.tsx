...
const SCORING_THRESHOLDS = {
  vix: {
    stress:      30,   // VIX > 30 : stress élevé
    caution:     25,   // VIX > 25 : prudence
    elevated:    20,   // VIX > 20 : légèrement élevé
    calm:        15,   // VIX < 15 : environnement serein
    extreme:     35,   // VIX > 35 : stress extrême (bloquant)
  },
  rsi: {
    overbought:  70,   // RSI > 70 : surachat
    overboughtX: 75,   // RSI > 75 : surachat fort
    overboughtXX:78,   // RSI > 78 : surachat extrême
    oversold:    30,   // RSI < 30 : survente
    oversoldX:   25,   // RSI < 25 : survente forte
    neutral_hi:  65,   // RSI > 65 : zone haute
    neutral_lo:  35,   // RSI < 35 : zone basse
    caution_lo:  40,   // RSI < 40 : zone basse modérée
    mid:         55,   // RSI < 55 : sous la médiane
  },
  fearGreed: {
    extremeFear:  15,  // F&G <= 15 : peur extrême forte
    fearX:        20,  // F&G < 20  : peur extrême modérée
    fear:         25,  // F&G < 25  : peur
    fearMod:      30,  // F&G <= 30 : peur modérée
    neutral_lo:   45,  // F&G <= 45 : neutre bas
    fifty:        50,  // F&G < 50  : sous la médiane
    neutral_hi:   55,  // F&G <= 55 : neutre haut
    greed:        75,  // F&G <= 75 : greed
    greedMod:     70,  // F&G >= 70 : greed modéré
...
type TrendPhase = {
  ...
  label:      "Accumulation" | "Breakout" | "Suivi de tendance" | "Pull-back" | "Excès final" | null;
  note:       string;
...
function detectTrendPhase(
  ...
  const hasSignal = (lbl: string) => signals.some(s => s.label === lbl || s.label.startsWith(lbl));
  const hasGoldenCross = hasSignal("Golden Cross");
  const rsiSignal      = signals.find(s => s.label.startsWith("RSI"));
  ...
  // Régression locale (approximation σ via écart-type des 60 dernières clôtures)
  const window60 = validCloses.slice(-60);
  const mean60   = window60.length > 0 ? window60.reduce((a, b) => a + b, 0) / window60.length : last;
  const std60    = window60.length > 1
    ? Math.sqrt(window60.reduce((s, v) => s + (v - mean60) ** 2, 0) / window60.length)
    : 0;
  const zScore   = std60 > 0 ? (last - mean60) / std60 : 0;

  // EMA50 approximation (20 dernières clôtures — proxy rapide)
  const ema50closes = validCloses.slice(-50);
  const ema50proxy  = ema50closes.length > 0
    ? ema50closes.reduce((a, b) => a + b, 0) / ema50closes.length
    : last;
  const distEMA50pct = ema50proxy > 0 ? ((last - ema50proxy) / ema50proxy) * 100 : 0;

  const isTendance  = context.type === "tendance";
  const isRange     = context.type === "range";
  const isBullDir   = context.structure.type !== "bearish";
  const sw           = sinewave;
  ...
  if (isTendance && isBullDir
    ...
    return {
      phase: 4, label: "Pull-back",
      note: `Phase 4 — Pull-back : prix revenu au contact de l'EMA50 (écart ${distEMA50pct.toFixed(1)}%) dans une tendance haussière préservée. Zone de renforcement selon la méthodologie PRO (signal DIV cachée ou BULL).`,
      confidence: "medium",
    };
  }

  // ── Phase 2 — Breakout ──
  // Transition récente vers tendance haussière + Golden Cross + ADX en hausse
  const isBreakoutMaturity = context.maturity === "jeune" || context.maturity === "en_developpement";
  if (isTendance && isBullDir && (isBreakoutMaturity || hasGoldenCross)) {
    const conf: TrendPhase["confidence"] = isBreakoutMaturity && hasGoldenCross ? "high" : "medium";
    return {
      phase: 2, label: "Breakout",
      note: `Phase 2 — Breakout : tendance ${isBreakoutMaturity ? "jeune" : "confirmée par Golden Cross"}. Les targets de break seront généralement débordées en tendance. Favoriser le suivi plutôt que les prises de profit prématurées.`,
      confidence: conf,
    };
  }

  // ── Phase 1 — Accumulation ──
  // Range mature + Sinewave en creux + contexte global haussier (Golden Cross ou EMA50 > EMA200 proxy)
  const isRangeMature = isRange && (context.maturity === "mature" || context.maturity === "divergence");
  if (isRangeMature && sw?.cycleTurn === "trough" && (hasGoldenCross || distEMA50pct > -10)) {
    return {
      phase: 1, label: "Accumulation",
      note: `Phase 1 — Accumulation : range mature avec creux de cycle Sinewave. Zone spéculative favorable pour les profils agressifs — signal BULL attendu pour valider l'entrée.`,
      confidence: "medium",
    };
  }

  // Tendance active sans phase précise identifiée
  if (isTendance) {
    return {
      phase: null, label: null,
      note: `Tendance active — phase non déterminée avec précision (confidence insuffisante).`,
      confidence: "low",
    };
  }
...
function scoreSentimentPressure(
  ...
  // VIX contextuel
  if (vix != null) {
    if (vix > SCORING_THRESHOLDS.vix.stress) {
      score -= 1.0;
    ...
    } else if (vix < SCORING_THRESHOLDS.vix.calm) {
      score += 0.5;
...
function calcConfluenceScore(
  ...
  const nearEma200   = (ema200 != null && last != null)
    ...
    : false;
  const rsiExtreme   = rsi != null && (rsi < SCORING_THRESHOLDS.rsi.neutral_lo || rsi > SCORING_THRESHOLDS.rsi.neutral_hi);
...
function calcCyclePhase(closes: (number|null)[]): CyclePhaseResult | null {
  ...
  const phase: "rising" | "peak" | "falling" | "trough" =
    lastRsi > SCORING_THRESHOLDS.rsi.neutral_hi && rsiSlope <= 0  ? "peak"    :
    lastRsi < SCORING_THRESHOLDS.rsi.neutral_lo && rsiSlope >= 0  ? "trough"  :
    rsiSlope > 2                   ? "rising"  :
    rsiSlope < -2                  ? "falling" :
...
function MacroContextPanel({ macro, zone }: { macro: MacroContext | null | undefined; zone?: MacroZone }) {
  ...
  const vixSignal = macro.vix == null ? null
    : macro.vix > SCORING_THRESHOLDS.vix.stress
    ...
    : macro.vix > SCORING_THRESHOLDS.vix.elevated
...
function computeEntryRecommendation(
  ...
  // ── Phase de tendance — enrichissement des reasons (non bloquant) ──
  const phaseNote = (trendPhase && trendPhase.confidence !== "low" && trendPhase.note)
    ? trendPhase.note
    : null;
  ...
  if (context.type === "chaos") {
    ...
    if (phaseNote) reasons.push(phaseNote);
    return {
      type: "wait", icon: "⛔",
...
function computeCryptoSentimentSynthesis(
  ...
  if (fg != null && fg >= SCORING_THRESHOLDS.fearGreed.greedMod && ls != null && ls > 1.6) {
    return {
      emoji: "🔶", color: "#f59e0b", bg: "#1a1000", border: "#f59e0b",
  ...
  if (fg != null && fg <= SCORING_THRESHOLDS.fearGreed.fearMod && ls != null && ls < 0.85 && fr != null && fr < 0) {
    return {
      emoji: "🔍", color: "#60a5fa", bg: "#111d30", border: "#60a5fa",
  ...
  if (fg != null && fg >= SCORING_THRESHOLDS.fearGreed.neutral_lo && fg <= SCORING_THRESHOLDS.fearGreed.neutral_hi && ls != null && ls >= 0.9 && ls <= 1.3) {
    return {
  ...
  if (fg != null) {
    const fgColor = fg <= SCORING_THRESHOLDS.fearGreed.fear ? "#ef4444" : fg <= SCORING_THRESHOLDS.fearGreed.neutral_lo ? "#f97316" : fg <= SCORING_THRESHOLDS.fearGreed.neutral_hi ? "#94a3b8" : fg <= SCORING_THRESHOLDS.fearGreed.greed ? "#f59e0b" : "#22c55e";
    const fgDesc  = fg <= SCORING_THRESHOLDS.fearGreed.fear ? "Peur extrême" : fg <= SCORING_THRESHOLDS.fearGreed.neutral_lo ? "Peur" : fg <= SCORING_THRESHOLDS.fearGreed.neutral_hi ? "Neutre" : fg <= SCORING_THRESHOLDS.fearGreed.greed ? "Greed" : "Extreme Greed";
    return {
      emoji: fg <= SCORING_THRESHOLDS.fearGreed.neutral_lo ? "😨" : fg <= SCORING_THRESHOLDS.fearGreed.neutral_hi ? "😐" : "😄",
      color: fgColor, bg: THEME.bgCard, border: fgColor,
      badge: fgDesc,
      title: `Fear & Greed à ${fg}/100 — ${fgDesc}`,
      body: `${ls != null ? `Ratio Long/Short : ${ls.toFixed(2)} (${lp?.toFixed(1) ?? "—"}% longs). ` : ""}${fr != null ? `Funding : ${fr > 0 ? "+" : ""}${(fr*100).toFixed(4)}%. ` : ""}${fg <= SCORING_THRESHOLDS.fearGreed.fear ? "Le marché est dans un état de peur prononcée — surveiller un signal technique de retournement pour envisager une entrée." : fg >= SCORING_THRESHOLDS.fearGreed.greed ? "L'optimisme est élevé — la marge de sécurité pour entrer est réduite." : "Sentiment intermédiaire — croiser avec l'analyse technique."}`,
    };
  }
                ...
                {fearGreed != null && (
                  <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:4, minWidth:130 }}>
                    ...
                    <div style={{ fontSize:12, fontWeight:700, color:
                      fearGreed.value < SCORING_THRESHOLDS.fearGreed.fearMod ? THEME.scoreRed : fearGreed.value < SCORING_THRESHOLDS.fearGreed.fifty ? "#f97316" :
                      fearGreed.value < SCORING_THRESHOLDS.fearGreed.greedMod ? THEME.scoreAmber : THEME.scoreGreen
                    }}>{fearGreed.label}</div>
